 ...
 * @extends  WC_Widget
 */
class Eva_Widget_Attributes_Filter extends WC_Widget {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce eva_attributes_filter widget_layered_nav';
		$this->widget_description = esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'eva' );
		$this->widget_id          = 'eva_attributes_filter';
		$this->widget_name        = esc_html__( 'Eva Attributes Filter', 'eva' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'      => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by', 'eva' ),
				'label' => esc_html__( 'Title', 'eva' )
			),
			'attribute'  => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'eva' ),
				'options' => $attribute_array
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'eva' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'eva' ),
					'or'  => esc_html__( 'OR', 'eva' )
				)
			),
		);
	}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $show_color ) {
		...
		foreach ( $terms as $term ) {
			...
			echo apply_filters( 'eva_color_filter_nav_count', '<span class="count">' . absint( $count ) . '</span>', $count, $term );
