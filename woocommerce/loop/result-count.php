...
if ( is_product_category() && (get_woocommerce_term_meta($parent_id, 'display_type', true) == 'subcategories' ) ) $show_ordering = TRUE;
...
<?php if ( $show_ordering == FALSE): ?>
    ...
    <p class="woocommerce-result-count">
        <?php
            ...
            if ( 1 == $total ) {
                echo esc_html__( 'Showing the single product', 'eva' );
            } elseif ( $total <= $per_page ) {
                printf( esc_html__( 'Showing all %d products', 'eva' ), $total );
            } else {
                printf( esc_html__( 'Showing %1$d-%2$d of %3$d products', 'eva' ), $first, $last, $total );
            }
        ?>
    </p>
    ...
    <?php if ( (isset($tdl_options['tdl_shop_pagination'])) && ($tdl_options['tdl_shop_pagination'] == "classic") ) : ?>
        ...
        $page_count = eva_loop_shop_per_page();
